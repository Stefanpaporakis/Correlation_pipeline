...
class path_maker:
    ...
    def load_correlation_path(self,init_path):
        ...
        if self.reduced_corr == True:
            ...
            corr_path = corr_path+analysis+"/"+str(self.lower)+"_"+str(self.upper)+"/reduced_"
        else:
            corr_path = corr_path 
        return corr_path
    ...
    def load_saxs_path (self, init_path):
        if self.reduced_corr == True:
            saxs_path = init_path+f"eiger/{self.group}/{self.tag}/corr_nps{self.chunksize}/"
            if self.analysis == 'max_value.npy':
                analysis = 'maxpeaks'
            if self.analysis == 'radial_peak_position.npy':
                analysis = 'radpeakpos'
            ...
            if self.analysis =='radial_peak_height.npy':
                analysis ='radpeakheight'
            saxs_path = saxs_path+analysis+"/"+str(self.lower)+"_"+str(self.upper)+"/"
        else:
...
class data_loader(path_maker):
	...
	def load_saxs_data(self,init_path):
		...
		#saxs = np.load(saxs_path+'_sum_red.npy')
		if self.reduced_corr == False:
			saxs2D = np.load(saxs_path+'summed_diffraction.npy')
			saxs1D = np.load(saxs_path+'1d_sum.npy')
		if self.reduced_corr == True:
			saxs2D = np.load(saxs_path+'reduced_diffraction.npy')
			saxs1D = np.load(saxs_path+'reduced_1d_sum.npy')
		bg = False
		if bg== True:
			bg2D = np.load(init_path+f'eiger/calibration/138017_8/summed_diffraction.npy')
			saxs2D = saxs2D-bg2D
			bg1D = np.load(init_path+f'eiger/calibration/138017_8/138017_8_sum_red.npy')
			saxs1D[:,1] = saxs1D[:,1]-bg1D[:,1]
		else:
			pass
		return saxs2D,saxs1D
		
	def load_correlation_data(self,init_path,qslice,width,angle_blur):
		corr_path = self.load_correlation_path(init_path)
		full = np.load(corr_path+"bgsub_"+self.tag+"_nstart400_correlation_sum.npy")
	...
	def load_3D_correlation_data(self,init_path,qslice,width,angle_blur):
		corr_path = self.load_correlation_path(init_path)
		full = np.load(corr_path+"3D_bgsub_"+self.tag+"_nstart400_correlation_sum.npy")
		return full
	...
	def load_all(self,init_path,qslice,width,angle_blur):
		saxs2d,saxs1d = self.load_saxs_data(init_path)
		...
		return saxs2d,saxs1d,corrd,sumd,maxd,radppd,radphd
